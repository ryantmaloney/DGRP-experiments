  ...
  for (i in 1:S){//Calculate separately for each strain
    for (j in 1:N){ //Calculate separately for each fly
      if (s[j]==i){ //If the strain of the fly matches the strain we're on
        R[j,1] ~ normal(0, D[i]+BH[i]); //Initial preference is given by BH
        // R[j,1] ~ normal(.5, B[i]);
        for (t in 2:T){
          // R[j, t] ~ normal(.5, BH[i]);
          R[j, t] ~ normal(R[j, t-1], D[i]); //Each day it changes by 
          // R[j,t] ~ normal(.5, B[i]);
          R[j,t] ~ normal(.5, B[i]*BH[i]*(BH[i]*D[i])/(BH[i]+D[i]-B[i]*BH[i]));// Scale so bound measures convergence or divergence
